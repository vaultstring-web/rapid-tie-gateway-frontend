...
import { Calendar, Download, Filter, TrendingUp, Users, Ticket, DollarSign, BarChart3, PieChart, MapPin, Sun, Moon } from 'lucide-react';
...
export default function EventAnalyticsPage() {
  ...
  return (
    <div className="min-h-screen p-6 transition-colors duration-300" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header with Theme Toggle */}
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold transition-colors duration-300" style={{ color: 'var(--text-primary)' }}>
              Event Analytics
            </h1>
        ...
        {/* Filters */}
        <div className="rounded-xl p-4 border transition-colors duration-300" style={{
          backgroundColor: 'var(--bg-secondary)',
          borderColor: 'var(--border-color)',
        }}>
          <div className="flex flex-wrap gap-4 items-center justify-between">
            <div className="flex flex-wrap gap-4 items-center">
              <div className="flex items-center gap-2">
                <Filter size={16} className="transition-colors duration-300" style={{ color: 'var(--text-secondary)' }} />
            ...
            <div className="flex items-center gap-2 text-sm transition-colors duration-300" style={{ color: 'var(--text-secondary)' }}>
              <Calendar size={14} />
        ...
        {/* Tab Navigation */}
        <div className="flex gap-2 border-b transition-colors duration-300" style={{ borderColor: 'var(--border-color)' }}>
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className={`${card.bg} p-3 rounded-lg transition-colors duration-300`}>
                ...
                <div className="mt-4">
                  ...
                  <p className="text-2xl font-bold mt-1 transition-colors duration-300" style={{ color: 'var(--text-primary)' }}>
                    {card.value}
                  </p>
                </div>
              </div>
        ...
        {activeTab === 'overview' && (
          <>
            ...
            >
              <h2 className="text-lg font-semibold mb-4 transition-colors duration-300" style={{ color: 'var(--text-primary)' }}>
            ...
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b transition-colors duration-300" style={{ borderColor: 'var(--border-color)' }}>
                      <th className="text-left py-3 font-medium transition-colors duration-300" style={{ color: 'var(--text-secondary)' }}>
                        Role
                      </th>
                      <th className="text-right py-3 font-medium transition-colors duration-300" style={{ color: 'var(--text-secondary)' }}>
                        Views
                      </th>
                      <th className="text-right py-3 font-medium transition-colors duration-300" style={{ color: 'var(--text-secondary)' }}>
                        Purchases
                  ...
                  <tbody>
                    {data.roleAnalytics.map((role) => (
                      <tr key={role.role} className="border-b transition-colors duration-300" style={{ borderColor: 'var(--border-color)' }}>
                        <td className="py-3 font-medium transition-colors duration-300" style={{ color: 'var(--text-primary)' }}>
                          {role.role}
                        </td>
                        <td className="py-3 text-right transition-colors duration-300" style={{ color: 'var(--text-secondary)' }}>
                        ...
                        <td className="py-3 text-right font-medium">
                          <span className="text-primary-green-600 dark:text-primary-green-400">
