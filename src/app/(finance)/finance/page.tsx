...
export default function FinanceDashboard() {
  const pendingDisbursements =
    MOCK_DISBURSEMENTS?.filter((d: { status: string }) => d.status === 'Ready').length || 0;
  const activeBatches = MOCK_BATCHES?.filter((b: Batch) => b.status === 'Processing').length || 0;

  const budgetData =
    MOCK_BUDGETS?.map((b: Budget) => ({
      name: b.department,
      spent: b.spent,
    ...
    <div className="space-y-8 animate-slide-up">
      <header>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Overview of budget utilization, disbursements, and reconciliation status.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card border-l-4 border-[#84cc16]">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs font-bold text-gray-400 dark:text-gray-300 uppercase tracking-wider">
          ...
          <Link
            href="/finance/disbursements/ready"
            className="text-[10px] text-[#84cc16] font-bold uppercase mt-4 flex items-center gap-1 hover:underline"
          >
        ...
        <div className="card border-l-4 border-blue-500">
          ...
          <Link
            href="/finance/disbursements/batches"
            className="text-[10px] text-blue-600 font-bold uppercase mt-4 flex items-center gap-1 hover:underline"
          >
        ...
        <div className="card border-l-4 border-red-500">
          ...
          <Link
            ...
            className="text-[10px] text-red-600 font-bold uppercase mt-4 flex items-center gap-1 hover:underline"
          >
      ...
      <div className="card overflow-hidden">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/finance/disbursements/batches" className="btn-secondary btn-small">
            View All Batches
          </Link>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-700">
                <th className="pb-3 text-xs font-bold text-gray-400 dark:text-gray-300 uppercase">
                  Batch Name
                </th>
                <th className="pb-3 text-xs font-bold text-gray-400 dark:text-gray-300 uppercase">
                  Status
                </th>
                <th className="pb-3 text-xs font-bold text-gray-400 dark:text-gray-300 uppercase text-right">
                  Items
                </th>
                <th className="pb-3 text-xs font-bold text-gray-400 dark:text-gray-300 uppercase text-right">
                  Total Amount
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
              {MOCK_BATCHES?.slice(0, 3).map((batch: Batch) => (
                ...
                >
                  <td className="py-4">
                    <div className="font-bold text-gray-900 dark:text-white">{batch.name}</div>
                    <div className="text-[10px] text-gray-400 dark:text-gray-500">{batch.id}</div>
                  </td>
                  <td className="py-4">
                    <span
                      className={cn(
                  ...
                  <td className="py-4 text-right font-medium text-gray-600 dark:text-gray-400">
                    {batch.itemCount}
                  </td>
                  <td className="py-4 text-right font-bold text-gray-900 dark:text-white">
                    MK {batch.totalAmount.toLocaleString()}
                  </td>
                  <td className="py-4 text-right text-gray-500 dark:text-gray-400 text-xs">
