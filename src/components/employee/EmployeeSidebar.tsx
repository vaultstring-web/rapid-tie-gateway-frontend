...
import { LayoutDashboard, Calendar, FileText, History, Settings, LogOut, ChevronLeft, ChevronRight, User, CreditCard } from 'lucide-react';
...
export function EmployeeSidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
  const pathname = usePathname();

  const navItems = [
    { name: 'Dashboard', href: '/employee', icon: LayoutDashboard },
    { name: 'My DSA Requests', href: '/employee/dsa/requests', icon: FileText },
    { name: 'New Request', href: '/employee/dsa/request', icon: Calendar },
    { name: 'Payment History', href: '/employee/payments', icon: History },
  ...
  return (
    ...
    >
      ...
      <div className={`h-16 flex items-center px-4 border-b border-[var(--border-color)] ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
        {!isCollapsed && (
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-sm text-[var(--text-primary)]">Employee Portal</span>
          </div>
        )}
        {isCollapsed && (
          <img src="/vault.png" alt="VaultString" className="w-8 h-8 object-contain" />
      ...
      <div className={`absolute bottom-0 w-full p-4 border-t border-[var(--border-color)] bg-[var(--bg-secondary)] ${isCollapsed ? 'flex justify-center' : ''}`}>
        <button className={`flex items-center gap-3 px-3 py-3 rounded-xl text-red-500 hover:bg-red-500/10 transition-colors ${!isCollapsed ? 'w-full' : ''}`}>
          <LogOut size={20} />
