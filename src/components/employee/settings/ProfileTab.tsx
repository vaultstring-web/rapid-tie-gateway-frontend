...
import { User, Mail, Phone, Building2, MapPin, Key, Camera, Save, Briefcase, Calendar } from 'lucide-react';
import { EmployeeProfile } from '@/types/employee/settings';
import { useTheme } from '@/context/ThemeContext';
import toast from 'react-hot-toast';

interface ProfileTabProps {
  profile: EmployeeProfile;
  onUpdate: (data: Partial<EmployeeProfile>) => Promise<void>;
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          <MapPin size={16} className="absolute left-3 top-3 text-[var(--text-secondary)]" />
          <textarea
            value={formData.address || ''}
            onChange={(e) => setFormData(prev => ({ ...prev, address: e.target.value }))}
            rows={2}
            className="w-full pl-10 pr-3 py-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-[#84cc16] resize-y"
            style={{
              backgroundColor: 'var(--bg-primary)',
              borderColor: 'var(--border-color)',
              color: 'var(--text-primary)',
