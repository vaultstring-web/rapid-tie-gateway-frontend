...
import { MatchingEvent } from '@/types/employee/dsa';
import { formatDate } from '@/lib/utils/format';
import { useTheme } from '@/context/ThemeContext';
import Link from 'next/link';

interface EventsDuringStayProps {
  events: MatchingEvent[];
  loading?: boolean;
}

export const EventsDuringStay = ({ events, loading }: EventsDuringStayProps) => {
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-[var(--text-secondary)]">No events found during your stay</p>
        <p className="text-xs text-[var(--text-secondary)] mt-1">Try adjusting your travel dates</p>
      </div>
    );
  }
